...
def generate_launch_description():
    ...
    manager = TimerAction(period=8.0, actions=[
        Node(package="nav2_lifecycle_manager", executable="lifecycle_manager",
             name="lifecycle_manager_navigation", output="screen",
             parameters=[{"use_sim_time": True, "autostart": True,
                          "bond_timeout": 0.0, "attempt_respawn_reconnection": False,
                          "node_names": NODES}]),
    ])
    rviz = TimerAction(period=2.0, actions=[
        ExecuteProcess(cmd=["rviz2", "-d", RVIZ], output="log"),
    ])
    return LaunchDescription(servers + [manager, rviz])
